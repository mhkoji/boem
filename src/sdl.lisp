...
(defmethod boem.rendering::sync-tile ((display display) tile x y)
  (with-accessors ((w display-w)
                   (h display-h)
                   (surface display-surface)) display
    (boem.rendering::do-tile-pixels ((pixel i j) tile)
      (let ((pos-x (+ (* x boem.rendering::*tile-pixel-count*) i))
            (pos-y (+ (* y boem.rendering::*tile-pixel-count*) j))
...
(defmethod boem.rendering::mem8-get ((memory list) (addr integer))
  (nth addr memory))
      ...
      (:idle ()
        ...
        (let ((display (make-display
              ...
              (memory '(#xFF #x00 #x7E #xFF
                        ...
                        #x93 #x85 #xA5 #x8B
                        #xC9 #x97 #x7E #xFF)))
          (boem.rendering::sync display memory))
        (sdl:update-display)))))
