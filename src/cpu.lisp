...
(defun reg8->place (register-set reg8)
  (make-place8-reg :register-set register-set :reg8 reg8))
...
(defun op-add (place register-set int8)
...
(defun op-sub (place register-set int8)
  (let* ((old-val (place8-get place))
         (new-val (- old-val int8)))
    (assert (<= 0 int8 #xFF))
    (assert (<= 0 old-val #xFF))
...
(defun reg8-add (register-set reg8 int8)
  (op-add (reg8->place register-set reg8) register-set int8))          

(defun reg8-sub (register-set reg8 int8)
  (op-sub (make-place8-reg :register-set register-set :reg8 reg8)
          register-set
          int8))
...
(defmethod run ((op addr8r8) memory register-set)
  (with-slots (x y) op
    (op-add (reg8->place register-set x) register-set (reg8-get register-set y)))
  (pc-inc register-set))

(defmethod run ((op addr8d8) memory register-set)
  (with-slots (r d) op
    (op-add (reg8->place register-set r) register-set d))
  (pc-inc register-set))

(defmethod run ((op addr8hl) memory register-set)
  (with-slots (r) op
    (let ((int8 (mem8-get memory (hl-get register-set))))
      (op-add (reg8->place register-set r) register-set int8)))
  (pc-inc register-set))

(defmethod run ((op adcr8r8) memory register-set)
  (with-slots (x y) op
    (let ((int8 (+ (reg8-get register-set y) (cy-get register-set))))
      (op-add (reg8->place register-set x) register-set int8)))
  (pc-inc register-set))

(defmethod run ((op adcr8d8) memory register-set)
  (with-slots (r d) op
    (op-add (reg8->place register-set r) register-set (+ d (cy-get register-set))))
  (pc-inc register-set))
...
(defmethod run ((op inchl) memory register-set)
  (with-slots (r) op
    (op-add (make-place8-mem :memory memory :addr (hl-get register-set))
            register-set
            1))
  (pc-inc register-set))
